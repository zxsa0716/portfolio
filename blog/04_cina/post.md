...
![CINA 메인 페이지를 아래로 내리면 나오는 전체 파이프라인 흐름도입니다. 왼쪽부터 데이터 수집, LLM 입장 추출, 그래프 분석, 브리핑 생성, 평가까지 다섯 단계가 이어지고, 각 단계에서 무엇이 산출되는지가 표시됩니다. 웹에서는 이 도식에 애니메이션이 걸려 데이터가 흐르는 것처럼 보입니다.](figures/web02.png)
...
방법론과 결과도 각각 별도 페이지로 정리했습니다.

![Methodology 페이지입니다. 3단계 파이프라인의 각 단계를 인터랙티브하게 설명하는데, 어떤 데이터가 들어가고 어떤 처리를 거쳐 무엇이 나오는지를 단계별로 펼쳐볼 수 있습니다. 논문의 방법론 절을 웹으로 옮긴 셈입니다.](figures/site_methodology.png)

![Visualizations 페이지입니다. 앞에서 보여드린 분석 그림들을 한곳에 모아 탐색할 수 있게 만들었습니다. 각 그림이 어떤 검정과 연결되는지도 함께 표시됩니다.](figures/site_visualizations.png)

![Outputs 페이지입니다. 이 프로젝트가 만들어낸 산출물(브리핑 문서, 데이터셋, 평가 보고서 등)을 목록으로 정리해 두었습니다. 결과물을 흩어놓지 않고 한 페이지에서 접근할 수 있게 하는 것도 재현성의 일부라고 생각했습니다.](figures/site_outputs.png)

![메인 페이지의 사례 분석 구간입니다. 한국의 적응정책 정합도와 브라질의 번역 격차를 웹에서 바로 확인할 수 있게 정리했고, 각 수치가 어떤 계산에서 나왔는지 함께 설명합니다.](figures/web03.png)

![메인 페이지 아래쪽의 검증·평가 구간입니다. 네 가지 과제(입장 정확도, 연합 탐지, 결과 예측, 브리핑 품질)의 성능과 판정 기준이 나란히 정리되어 있습니다. 통과 기준을 미리 밝히고 그 충족 여부를 표시하는 방식으로 구성했습니다.](figures/web04.png)
