...
![발표 표지입니다. '기후취약 계층 고령자 보호를 위한 정릉3동 빈집 활용 무더위쉼터 조성 방안'이라는 제목으로, 국민대학교 산림환경시스템학과와 정치외교학과가 함께 진행한 연구입니다.](figures/p01.png)
...
![연구 배경입니다. 왼쪽부터 온열질환에 취약한 고령층 비율, 무더위쉼터 인프라 부족, 주택 노후화와 빈집 증가의 세 가지 조건을 정리했습니다. 세 조건이 한 동네에 겹쳐 있다는 것이 이 연구를 시작한 이유입니다.](figures/p03.png)
...
![폭염 피해 증가 전망과 대응 필요성입니다. 왼쪽은 폭염일수 증가 전망이고, 오른쪽은 정릉3동의 지형·교통 조건과 무더위쉼터 현황입니다. 급경사지에 마을버스 1개 노선, 14,000명 주민에 쉼터 3곳이라는 조건이 정리되어 있습니다.](figures/p04.png)
...
![연구 목표와 방법입니다. 현장조사, 인터뷰·설문, 사례연구, 정책분석의 네 가지를 병행했습니다. 데이터만으로는 알 수 없는 실제 이용 행태를 파악하기 위해 현장 접근을 택했습니다.](figures/p05.png)
...
![정릉3동 고령층 인터뷰 결과입니다. 왼쪽은 기본 통계(11명 중 인지 8명, 이용 경험 1명)이고, 오른쪽 표는 인터뷰 대상별 이용 여부와 제한 사유를 정리한 것입니다. 경사와 거리 문제로 접근이 어렵고, 기존 실버복지센터나 복지관을 선호하는 경향이 확인되었습니다.](figures/p06.png)
...
![정릉종합사회복지관 담당자 서면 인터뷰 내용입니다. 지속성과 사후관리의 중요성을 강조한 답변이 담겨 있습니다. 이 인터뷰가 제안의 방향을 '설계'에서 '운영까지'로 확장시켰습니다.](figures/p07.png)
...
![인터뷰와 현장조사를 통해 도출된 주요 문제점 세 가지입니다. 빈집의 입지 한계, 서울시 성공사례와의 격차, 공공기관 협조 문제가 정리되어 있습니다. 연구가 뜻대로 되지 않은 부분까지 그대로 기록했습니다.](figures/p08.png)
...
![성북구 빈집 현황과 등급별 분포입니다. 왼쪽 원그래프에서 1~2등급이 63%를 차지하는 것을 볼 수 있고, 오른쪽에는 빈집 재생사업 현황과 소유주 인터뷰가 있습니다. 구는 철거비를 부담하고 수익은 소유자에게 환원하는 공공-민간 상생 모델이 이미 작동하고 있었습니다.](figures/p09.png)
...
![기존 사업의 한계와 무더위쉼터 조성의 필요성입니다. 주차장과 무더위쉼터의 목적·이용방식 차이를 비교하고, 예상되는 우려(소유주 비협조, 예산 제약, 주민 갈등, 안전성)에 대한 대응 논리를 정리했습니다. 무더위쉼터는 취약계층 생명 보호라는 명확한 공익적 목적이 있어 임대주택과 달리 접근할 수 있다고 봤습니다.](figures/p10.png)
...
![노원구 편백경로당 그린리모델링 우수사례입니다. 단순 기술 적용이 아니라 고령자 눈높이에 맞춘 설계로 성공한 사례로, 저희 설계의 참고 모델이 되었습니다.](figures/p13.png)

설계는 세 갈래로 구성했습니다.

**첫째, 패시브 설계.** 에너지를 쓰기 전에 **에너지가 필요 없게** 만드는 방식입니다. 단열 강화, 차양, 자연 환기를 통해 냉방 부하 자체를 줄입니다. 돈이 들지 않는 냉방인 셈입니다.

![패시브 설계 방안입니다. 고단열·고기밀 외피, 차양 계획, 자연환기 경로 등을 통해 냉방 부하를 근본적으로 줄이는 전략을 정리했습니다. 건물이 스스로 시원해지도록 만드는 것이 가장 저렴한 냉방입니다.](figures/p17.png)

**둘째, 액티브 설계.** 그래도 필요한 냉방은 고효율 설비로 해결합니다.

![액티브 설계 방안입니다. 고효율 냉방설비와 LED 조명 등 실제 가동되는 설비 계획입니다. 패시브로 부하를 줄인 뒤 남은 필요를 효율적으로 채우는 순서로 설계했습니다.](figures/p18.png)

**셋째, 신재생에너지.** 태양광 발전으로 운영 전력을 스스로 만듭니다.

![신재생에너지와 기타 설비 계획입니다. 태양광 발전 시스템과 관련 설비를 통해 에너지 자립도를 높이는 구성입니다.](figures/p19.png)
...
![에너지 측면 기대효과입니다. 에너지 자립률 목표와 탄소 저감 효과가 함께 정리되어 있습니다. 폭염 대응(적응)과 온실가스 감축(완화)을 동시에 달성하는 구조라는 점이 이 설계의 특징입니다.](figures/p20.png)

공간 계획도 실제 이용을 고려했습니다. 단순히 에어컨 있는 방 하나가 아니라, 어르신들이 하루를 보낼 수 있는 구성으로 짰습니다.

![시설 구성 및 공간 계획입니다. 휴식 공간, 소통 공간, 다목적 공간 등으로 나누어 실제 체류가 가능한 구성을 제안했습니다. 인터뷰에서 확인한 "복지관이 더 쾌적하다"는 의견을 반영해 시설 품질을 중요하게 다뤘습니다.](figures/p21.png)
...
![지역 연계 및 협력 체계입니다. 복지관과 주민 조직, 자원봉사를 연결한 운영 모델을 제안했습니다. 시설을 짓는 주체와 운영하는 주체를 분리해 지속성을 확보하는 구조입니다.](figures/p22.png)

![이해관계자 분석과 협력 체계 구축 방안입니다. 성북구, 빈집 소유주, 지역 주민, 복지 기관 각각의 이해관계와 역할을 정리했습니다. 어느 하나라도 빠지면 사업이 멈춘다는 것을 현장에서 배웠기 때문에 넣은 부분입니다.](figures/p12.png)

그리고 예산안도 산정했습니다. 아이디어만 제시하고 "예산은 알아서" 하는 제안은 실현되지 않습니다.

![예상 예산안입니다. 항목별 비용과 재원 조달 방안을 정리했습니다. 초기 조성비와 연간 운영비를 구분해 산정했습니다.](figures/p25.png)
...
![결론 슬라이드입니다. 기후취약 고령층 보호, 지역사회 복지 연계, 에너지 전환의 세 축에서 기대효과를 정리하고, 제도화와 확산 경로를 제안했습니다.](figures/p27.png)
