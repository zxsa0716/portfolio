...
![논문 Figure 1입니다. 왼쪽 두 개의 작은 지도는 동아시아와 한반도에서 서울의 위치를 보여주고, 오른쪽 큰 지도가 실제 분석 대상입니다. 서울 전역이 500m 격자로 촘촘하게 덮여 있고(붉은 격자선), 색으로 다섯 개 기능 권역을 구분했습니다. 진한 자주색이 도심권(중구·종로구), 붉은색이 동북권, 노란색이 서북권, 파란색이 서남권, 자홍색이 동남권입니다. 이 다섯 권역 구분이 뒤에서 나오는 핵심 발견의 무대가 됩니다.](figures/01.png)
...
![논문 Figure 2입니다. IPCC AR6의 위험 개념을 이 연구의 구조로 옮긴 그림입니다. 가운데 흰 원이 '도시 폭염 위험'이고, 그 주변을 위해(Hazard, 남색)·노출(Exposure, 하늘색)·취약성(Vulnerability, 파란색) 세 타원이 겹치며 감싸고 있습니다. 취약성은 다시 사회적 민감도(주황)와 적응 역량(초록)으로 나뉩니다. 회색 선으로 이어진 점들과 그 위의 α 기호가 그래프 어텐션 네트워크가 학습하는 '요소 간 연결 강도'입니다. 즉 세 요소를 단순히 더하지 않고, 어느 연결이 얼마나 중요한지를 모델이 직접 배우게 만들었다는 뜻입니다.](figures/02.png)

각 요소에 변수를 네 개씩, 총 **16개 변수**를 배정했습니다. 기상청, 통계청, 서울시 등의 공공데이터를 썼습니다.

![논문 Table 1입니다. 분석에 쓴 16개 변수를 AR6 요소별로 정리한 표입니다. 기후 위해에는 폭염일수·열대야일수·온열지속기간·지표면온도가, 도시 노출에는 인구밀도·불투수면비율·건물밀도·교통접근성이, 사회적 민감도에는 반지하주거비율·영유아·저소득·고령인구가, 적응 역량에는 여름철 냉방전력·의료접근성·녹지·무더위쉼터밀도가 들어갔습니다. 오른쪽 열에 각 변수의 출처 기관이 적혀 있는데, 전부 공개된 공공데이터라 다른 연구자도 같은 자료를 구할 수 있습니다.](figures/03.png)
...
![논문 Figure 4입니다. 똑같은 위험도를 두 가지 해상도로 그린 지도입니다. 왼쪽이 이 연구의 500m 격자 결과이고, 오른쪽이 그것을 자치구 단위로 평균 낸 결과입니다. 색이 진할수록 위험이 높습니다. 오른쪽 자치구 지도를 보면 구 전체가 하나의 색으로 뭉뚱그려져 있습니다. 반면 왼쪽 격자 지도를 보면 같은 구 안에서도 색이 확연히 다른 칸들이 섞여 있습니다. 공원과 산 주변은 옅고, 고밀 상업지역은 진합니다. 자치구 평균을 내는 순간 이 차이가 전부 사라진다는 것이 이 두 장의 비교가 말하는 바입니다.](figures/05.png)
...
![논문 Table 8입니다. 광역 축척에서 어떤 변수가 중요했는지를 순위로 정리하고, 세 XAI 기법의 합의율을 함께 표시했습니다. 위쪽 네 줄이 요소별 기여도인데 도시 노출 33.73%, 사회적 민감도 28.23%, 기후 위해 23.57%, 적응 역량 14.47% 순입니다. 아래는 개별 변수 순위로, 불투수면(0.368, 합의율 94.2%)·고령인구(0.341, 92.8%)·건물밀도(0.322, 91.3%)가 최상위 세 개입니다. 오른쪽 합의율이 모두 84% 이상이라는 점이 중요한데, 원리가 다른 세 기법이 이만큼 일치했다는 뜻입니다.](figures/06.png)
...
![논문 Figure 6입니다. 서울 다섯 권역에서 AR6 네 요소가 각각 얼마나 기여했는지를 막대로 나타냈습니다. 보라색이 기후 위해, 하늘색이 도시 노출, 초록색이 사회적 민감도, 노란색이 적응 역량입니다. 보시면 다섯 권역 전부에서 보라색 막대가 압도적으로 높습니다. 도심권 67.0%, 동북권 62.2%, 서북권 66.1%, 동남권 68.2%, 서남권 66.5%로 모두 60%를 넘습니다. 광역 축척에서 1위였던 도시 노출은 여기서 14~18%로 내려앉았습니다. 같은 데이터, 같은 모델인데 보는 축척만 바꾸니 주범이 바뀐 것입니다.](figures/08.png)
...
![논문 Table 10입니다. 여섯 개 건강 지표별로 거리상관, 95% 신뢰구간, p값, 최적 시차를 정리한 표입니다. 1인당 지표 세 개는 시차 0년에서 가장 높은 상관을 보이며 p < 0.001로 유의했고, 절대 건수 지표들은 시차 2~3년에서 최고값이 나왔습니다. 여섯 개 중 네 개가 p < 0.05 수준에서 유의했습니다. 통계적 견고성은 부트스트랩 1,000회와 순열검정으로 확인했습니다.](figures/10.png)
