...
![플랫폼의 기본 화면입니다. 상단에 제목이 있고, 그 아래 다섯 개 모듈로 이동하는 탭(정책 시뮬레이션, 알림 모니터링, 경제 파급효과, 업종별 위험도, 지역별 위험도 맵)이 배치되어 있습니다. 화면 아래쪽에는 서울 지도 위에 가맹점 위험도가 색깔 점으로 표시되어 있는데, 붉은 점이 고위험 점포입니다. 담당자가 어느 모듈이든 두 번의 클릭으로 도달할 수 있게 구조를 단순하게 유지했습니다.](figures/01.png)

![모듈 선택 화면입니다. 다섯 개 분석 기능을 카드 형태로 배치해, 지금 필요한 분석이 무엇인지에 따라 골라 들어갈 수 있게 했습니다. 처음 보는 사람도 무엇을 할 수 있는 도구인지 5초 안에 파악하도록 만드는 게 목표였습니다.](figures/02.png)
...
![정책 시뮬레이션 모듈입니다. 왼쪽 위 그래프는 정책별 위기 가맹점 수의 12개월 추이인데, 아무 개입이 없는 빨간 선은 계속 올라가고 종합 정책인 파란 선은 거의 평평합니다. 오른쪽 위는 12개월 뒤 최종 효과 비교 막대이고, 왼쪽 아래는 비용 대비 효과를 점으로 표현한 산점도, 오른쪽 아래는 최고 정책의 효율을 게이지로 표시한 것입니다. 담당자가 조건을 바꿔가며 이 네 개 그림이 함께 변하는 것을 볼 수 있게 배치했습니다.](figures/03.png)
...
![위험도 알림 모니터링 모듈입니다. 위험 등급별 현황과 주의가 필요한 상권 목록을 보여줍니다. 매일 확인하는 운영 화면의 성격에 맞춰, 새로 위험 등급이 올라간 항목이 먼저 보이도록 배치했습니다.](figures/04.png)
...
![경제 파급효과를 Sankey 다이어그램으로 표현한 모듈입니다. 왼쪽에서 시작된 충격이 오른쪽으로 번져가는 경로와 규모를 흐름의 굵기로 보여줍니다. 숫자표로는 전달이 어려운 '연쇄'라는 개념을 직관적으로 전달하기 위해 선택한 시각화입니다.](figures/05.png)
...
![업종별 리스크 프로파일 모듈입니다. 업종에 따라 위험 요인의 구성이 어떻게 다른지 비교해 보여줍니다. 일괄 지원보다 업종별 차별화 지원이 필요한 이유를 데이터로 설명하는 화면입니다.](figures/06.png)
...
![지역별 인터랙티브 맵 모듈입니다. 자치구 경계와 개별 점포 위험도를 함께 표시했습니다. 붉은 점이 몰려 있는 구역이 보이는데, 이런 군집은 개별 지원보다 권역 단위 대응이 효과적입니다.](figures/07.png)

![지도를 확대해서 본 화면입니다. 개별 점포 단위까지 위험도가 표시되고, 색으로 등급이 구분됩니다. 담당자가 자기 담당 구역을 찾아 바로 확인할 수 있게 만드는 것이 목표였습니다.](figures/08.png)
